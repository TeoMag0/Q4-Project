    ...
    public void onCollisionEnter(Collider col){
        ...
        if(col.purpose() == ColliderPurpose.WALL){
            ...
            if (circleCol != null) {
                ...
                float keepPlayerCollidingInset = .005f;
                if(Vector2.dot(normalForceDir, Vector2.right()) > 0){
                    //on the right side
                    player.moveTransform(Vector2.multiply(Vector2.right(), -keepPlayerCollidingInset+playerCol.radius()+wallCol.size().getX()/2 - Vector2.dot(Vector2.difference(getPos(),wallCol.getPos()), Vector2.right())));
                }else if(Vector2.dot(normalForceDir, Vector2.right()) < 0){
                    //on the left side
                    player.moveTransform(Vector2.multiply(Vector2.left(), -keepPlayerCollidingInset+playerCol.radius()+wallCol.size().getX()/2 - Vector2.dot(Vector2.difference(getPos(),wallCol.getPos()), Vector2.left())));
                }else if(Vector2.dot(normalForceDir, Vector2.up()) > 0){
                    //on top
                    player.moveTransform(Vector2.multiply(Vector2.up(), -keepPlayerCollidingInset+playerCol.radius()+wallCol.size().getY()/2 - Vector2.dot(Vector2.difference(getPos(),wallCol.getPos()), Vector2.up())));
                }else if(Vector2.dot(normalForceDir, Vector2.up()) < 0){
                    //on bottom
                    player.moveTransform(Vector2.multiply(Vector2.down(), -keepPlayerCollidingInset+playerCol.radius()+wallCol.size().getY()/2 - Vector2.dot(Vector2.difference(getPos(),wallCol.getPos()), Vector2.down())));
                }
            }else if(boxCol != null){
